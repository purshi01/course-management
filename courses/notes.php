...
<?php
SESSION_START();
$query = "SELECT * FROM `notes` WHERE `cid`='".$_SESSION['cid']."'";
$result = mysqli_query($data->con,$query);
while ($row = mysqli_fetch_assoc($result))
{
    $id = $row['cid'];
    $name = $row['nname'];
      echo "<center><h2 class='city'><a href='showpdf.php?id=$id' style='color:black' >".$name."&nbsp&nbsp&nbsp</a><a href='VandNdelete.php?nid=$id'><span class='glyphicon glyphicon-trash'></span></a></h2></center>";
